...
**Live updates — polling:** SSE was attempted but rejected by Cloudflare Tunnel ("stream canceled by remote, error code 0") — do not retry SSE while the app runs behind Cloudflare. Live updates are implemented via 5-second polling in aventyrsdagbok.js: `startPolling(adventureId)` / `stopPolling()` / `pollChapters(adventureId)`. `startPolling` is called after chapters load in `selectAdventure`. `stopPolling` is called at the start of `selectAdventure` and on `beforeunload`. Poll results are applied silently — never show errors to the user on poll failure. A chapter editor that is currently focused (`document.activeElement === editor`) is never overwritten by a poll result — the user's in-progress text is always protected. Identical content (same `bodyHtml`, `title`, `date`) is skipped — no unnecessary DOM updates.
